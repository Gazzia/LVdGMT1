...
function refImg(){
  $(".UIBG_img, .UIBG_imgFG, .UIBG_Prop").fadeOut(500, function(){
    ...
    if (page==2.1){
      ...
      $("#UIBG_imgFullFG").attr("src","images/Deco/full/FullFG_2.1.png");
    ...
    if (page==2.11){
      ...
      $("#UIBG_imgFull").attr("src","images/Deco/full/Full_2.11.png");
    }
    if (page==2.2){
    ...
    var UIPropStyle = Math.floor(Math.random() * (10 - 1 + 1) + 1);
    if (UIPropStyle <= 5){$('#UIBG_Prop').attr('src','images/Deco/prop/mouches3.gif');}
    else {$('#UIBG_Prop').attr('src','');}
    $('#UIBG_Prop').css({'left': Math.floor(Math.random() * (80 - 10 + 1) + 10)+'%','bottom': Math.floor(Math.random() * (40 - 2 + 1) + 2)+'%'});
    changeColor();
  });
  $(".UIBG_img, .UIBG_imgFG, .UIBG_Prop").fadeIn(300);
}
...
function changeColor(){
  ...
  if (localStorage.time == "SunnyAfternoon"){
    ...
    $(".UIBG_img").css({"filter":"hue-rotate(-10deg) saturate(105%)","transition":"filter 4s ease-in"});
