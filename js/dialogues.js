function talk(dial_Said) {
  ...
  if (localStorage.dial_Said == 0) {
    $(".dialSub0, #dialQuit").show();
  }
  if (localStorage.dial_Said != 0) {
    $(".dialSub" + localStorage.dial_Said + ".dialShown, #dialRetour, #dialQuit").show();
  }
  if (localStorage.région == "Soufflant" && localStorage.zone == "Guarde") {
    ...
    if (localStorage.dial_Said == 'B2A') {
      if (localStorage.plGold > 50) {
        localStorage.plGold -= 50;
      }
      if (localStorage.plGold <= 50) {
        localStorage.plGold = 0;
      }
  ...
  if (localStorage.numpage == 5.11) {
    ...
    if (localStorage.dial_Said == '0' && localStorage.talked_guy1 == 1) {
      ...
      if (oddguy1random == 1) {
        $("#histoire").html("\"Tu me fais de l'ombre.\"");
      }
      if (oddguy1random == 2) {
        $("#histoire").html("\"J'étais plutôt tranquille avant que tu ne me parles.\"");
      }
      if (oddguy1random == 3) {
        $("#histoire").html("\"Arrêtes de me dévisager.\"");
      }
      if (oddguy1random == 4) {
        $("#histoire").html("\"Laisses-moi en paix.\"");
      }
      if (oddguy1random == 5) {
        $("#histoire").html("\"Vas jouer ailleurs.\"");
      }
      $(".dialAll").hide();
      $("#dialQuit").show();
    }
